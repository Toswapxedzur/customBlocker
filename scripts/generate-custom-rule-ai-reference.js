...
function buildReference(eventSource, helperSource) {
  ...
  return [
    ...
    "PREDICATE_ITEM: url, name/title, author, length, views, publishedAt, description, live, sponsored, algorithmic, videoForm, tags. Many fields may be null; null-check and fail open. tags is an array (possibly empty) of {id, name, confidence} from the local Vault content classifier — content-based, not creator-based; confidence is 1..5 (0 = unknown). Gate on confidence to avoid acting on weak guesses, e.g. (item) => Array.isArray(item.tags) && item.tags.some(t => t.name === 'Gaming' && t.confidence >= 4).",
